...
export class SearchService {
  ...
  private groupRepositories(repositories: IRepositoryData[][]): IResponse[] {
    const counts: { [name: string]: { url: string; count: number } } =
      repositories.flat().reduce((result, data) => {
        const key = data.name;
        if (!result[key]) {
  ...
  async getRepositories(url: string): Promise<IResponse[]> {
    try {
      const contributors =
        await this.githubService.getRepositoryContributors(url);
      const repositories = await this.githubService.getUsersRepositories(
        contributors,
        url,
      );
