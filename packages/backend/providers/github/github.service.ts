...
import { GITHUB_HOST, MAX_CONCURRENT_REQUESTS, MAX_PER_PAGE } from "./constants";
...
export class GithubService {
  ...
  private async getUserRepositories(username: string, maxRetries: number = 3): Promise<IRepositoryData[]> {
    let retries = 0;
  ...
  public async getRepositoryContributors(url: string): Promise<IContributor[]> {
    ...
    try {
      ...
      if (error.message.includes('too large to list contributors for this repository')) {
        this.logger.info({
  ...
  public async getUsersRepositories(usernames: { login: string }[]): Promise<IRepositoryData[][]> {
    const result = []
    const chunks = chunkArray(usernames, MAX_CONCURRENT_REQUESTS);
    const fetchRepositoriesByChunks = async (chunk: { login: string }[]) => {
      const promises = chunk.map(username => this.getUserRepositories(username.login));
